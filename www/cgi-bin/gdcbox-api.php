...
//        echo "<li>cmd=lastlog</li>";
    ...
    // get a list with Name, ID and Description from all Devices
    case 'listdevices';
        
        $query = $pdo->query('SELECT name, description, id FROM devices');
        
        foreach($query as $row){
            // check this device for being an Actor
            $query2 = $pdo->query('SELECT value FROM device_configs '
                                   . ' WHERE device_id=' . $row['id'] 
                                   . " AND key='Actor'");                                  
            $res = $query2->fetch();
            $isActor= ($res && $res['value']=='yes') ? "yes" : "no";
            // append to results
            $result[]=array("id"=>$row['id'],"name"=>$row['name'],
                          "description"=>$row['description'],"isActor" =>$isActor);
        }
