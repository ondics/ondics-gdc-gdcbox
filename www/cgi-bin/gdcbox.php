...
<?php
    ...
    */
    ...
    if ($action!="main") {
        if (!in_array($action,$actionswithnoback))
            echo '<a href="' . $env["myurl"].'" data-icon="back" data-rel="back" data-ajax="false" >Back</a>';
        if (!in_array($action,$actionswithnohome))
            echo '<a href="' . $env["myurl"].'" data-icon="home" data-ajax="false" >Home</a>';
    }
    echo '<h1>GDCBox</h1>';
    echo '</div><!-- /header -->';
    ...
    case "main":
        ...
        if (!$row) {
            ...
            /*    
                ...
                echo "x = ((". $app_width." * " . ($appcount-1) . " ) % (width-180));";
                echo "linebreak = (width-180)-30;";
                echo 'alert("[x="+x+",width="+width+",linebreak="+linebreak+"]");';
                echo 'alert("x>linebreak="+(x > linebreak)); };';
                echo 'if (x > linebreak) { alert("linebreak");document.write("</tr><tr>");};';
                echo "</script>\n";
            ...
            */
                ...
                echo '<div class="app-cell" '.
                     'style="float:left; width:' . $app_width . 'px; background-color:#' . $bgcolor . '">';

                //echo '<a href="'.$env["myurl"].'?action=configuredevice&device_id='.$row['id'].'">';
                echo '<a href="' . $env["myurl"] . '?action=configuredevice&device_id=' . $row['id'];
                echo '" style="color:#' . $textcolor . ';">';
            
                echo '<div class="app-cell-top">';
                echo '<img src="' . $env["baseurl"] . '/gdcbox/apps/' . $row[0] . '/' . $row[0] . '.png" '.
                        " width=\"$app_image_size\" height=\"$app_image_size\">";
                echo '</div>';
        ...
        if ($row[0]>=1)
            echo '<p style="clear:both;">&nbsp;</p><p><a href="' . $env["myurl"] . '?action=makenewdevice">Make new Device</a></p>';
    ...
    case "appstore":
        ...
        if ($testmode) echo "<p>url=$appstore_url</p>";
        $ch = curl_init();
    ...
    case "show_system_info":
        ...
        $appfile=$row[0];
        require_once($env["apppath"] . "/" . $row[0] . "/" . $row[0] . ".inc");
